(function($) {
    ...
    $(window).on("load", function() {
        if ($('div.estimated_deliveryx').length > 0) {
            ...
            var day_today_au = date_today.toLocaleString('en-AU', {
                timeZone: 'Australia/Sydney',
                weekday: 'short'
            });
            var hr_today_au = date_today.toLocaleString('en-AU', {
                timeZone: 'Australia/Sydney',
                hourCycle: 'h24',
                hour: '2-digit'
            });
...
document.addEventListener("DOMContentLoaded", (event) => {
    ...
    // --- Performance Optimized AfterPay Price Update ---
    function updateAfterPayPrices() {
        // Cart AfterPay
        const cartDetails = document.querySelector('.ajax-cart__final-details');
        if (cartDetails) {
            const totalText = cartDetails.querySelector('.subtotal-price .cart-original-total')?.textContent;
            if (totalText) {
                ...
                afterPayElements.forEach(function(afterPayElement) {
                    const price = (totalNumber / 4 / 100).toFixed(2);
                    afterPayElement.innerHTML = `${currencySymbol.replace(/(IDR|Bds|EUR|ALL|DZD|AUD|XCD|BSD|BZD|USD|BOB|KYD|CRC|DKK|FJD|XPF|GBP|HKD|HUF|INR|IDR|ILS|JPY|MYR|MVR|MUR|PHP|PLN|QAR|SAR|XOF|SBD|LKR|SEK|CHF|TWD|THB|UAE)/g, '')} ${price}`;
                });
            }
        }

        // Product Page AfterPay
        const productPriceEl = document.querySelector('.product__section-details .price__pricing-group .price-item--regular');
        const productAfterPayStrong = document.querySelector('.product__section-details .after_pay_row_inner strong');
        if (productPriceEl && productAfterPayStrong) {
            const proafterPay = productPriceEl.textContent;
            const PrototalNumber = proafterPay.replace(/[^\d]/g, '');
            let ProcurrencySymbol = proafterPay.replace(/[0-9.,\s]/g, '');
            const price = (PrototalNumber / 4 / 100).toFixed(2);
            productAfterPayStrong.innerHTML = `${ProcurrencySymbol.replace(/(IDR|Bds|EUR|ALL|DZD|AUD|XCD|BSD|BZD|USD|BOB|KYD|CRC|DKK|FJD|XPF|GBP|HKD|HUF|INR|IDR|ILS|JPY|MYR|MVR|MUR|PHP|PLN|QAR|SAR|XOF|SBD|LKR|SEK|CHF|TWD|THB|UAE)/g, '')} ${price}`;
        }
    }

    // Use a single low-frequency observer for price changes instead of multiple high-frequency intervals
    const priceObserver = new MutationObserver(updateAfterPayPrices);
    const cartContainer = document.querySelector('#slideout-ajax-cart');
    if (cartContainer) {
        priceObserver.observe(cartContainer, { childList: true, subtree: true, characterData: true });
    }
    const productPriceContainer = document.querySelector('.product__section-details .price__pricing-group');
    if (productPriceContainer) {
        priceObserver.observe(productPriceContainer, { childList: true, subtree: true, characterData: true });
    }
    updateAfterPayPrices(); // Initial call
});
...
document.addEventListener("DOMContentLoaded", function() {
    ...
    function handleSwatchClick(event) {
        ...
        const itemsLeftText = document.querySelector('.quick_view_box .items_left_text') || document.querySelector('.product__info-wrapper .items_left_text') || document.querySelector('.items_left_text');
        const itemsLeftTextParnt = document.querySelector('.modal__container.modal--active .quick_view_box') || document.querySelector('.product__info-wrapper') || document.querySelector('.product-single__info');
        if (!itemsLeftText || !itemsLeftTextParnt) return;
        let displayText = '';
        if (inventory === 0) {
    ...
    function initSwatchHandlers() {
        ...
        swatches.forEach(input => {
            input.removeEventListener('click', handleSwatchClick);
        ...
        checked.forEach(input => handleSwatchClick({
            target: input
        }));
    }
...
(function() {
    ...
    // Reduced frequency of duty checkbox check to improve main thread availability
    setInterval(addDutyCheckbox, 2000);
})();